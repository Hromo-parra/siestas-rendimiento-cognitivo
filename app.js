...
function renderBatteryIntro() {
  page(`<section class="form-card compact centered"><p class="eyebrow">Batería ${phaseLabel(state.phase).toLowerCase()}</p><h1>Dos tareas breves</h1><div class="battery-list"><div><b>Atención sostenida</b><span>20 respuestas rápidas</span></div><div><b>Memoria de trabajo</b><span>36 estímulos 2-back</span></div></div><p>Cada tarea mostrará primero una página individual de instrucciones. Durante la medición, esas instrucciones ya no estarán visibles.</p><p>Trabaja en un lugar silencioso. Usa el mismo dispositivo y postura en ambas sesiones.</p><button class="button primary" data-action="run-battery">Continuar a las instrucciones</button></section>`);
}
